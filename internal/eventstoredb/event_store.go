...
func (e *eventStore) SaveEvents(ctx context.Context, streamID string, events []common.Event) error {
...
func (e *eventStore) LoadEvents(ctx context.Context, streamID string) ([]common.Event, error) {
	...
	events := make([]common.Event, 0, 100)
	for {
		...
		events = append(events, common.NewEventFromRecorded(event.Event))
	}
